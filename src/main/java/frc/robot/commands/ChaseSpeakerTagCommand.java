...
/*
 *  Drive to the  speaker tag ONLY by the angle (rotation) detected by vision, drive at the fixed speed to the goal
 *  this is similar to drive straight and draight straight to note in Teleswerver
 */



public class ChaseSpeakerTagCommand extends Command {
  ...
  public void execute() {
    ...
    // Adjust the fixed speed HERE
    double translationVal = -0.2;//-0.30;
    ...
    if( photonCamera != null) {
      ...
      if( results.hasTargets() ) {
            ...
            if( result != null) {
                    ...
                    // add the vision data
                    driveStraightAngle = driveStraightAngle - result.getYaw() ; // add or minus need test out, mine +4 ==> -6
    ...
    if( backDistanceIRSensorReading > 800 &&  vinniesError  < 3  && distanceRobotToAprilTag < 1.3 ) {
      PoseEstimatorSubsystem.setLEDColor(Color.kGold);
      isGoalReached = true;
      System.out.println("Done - IR Sensor = "+backDistanceIRSensorReading+", pigeon Yaw = "+drivetrainSubsystem.getYawInDegree()+", angle error = "+vinniesError);
