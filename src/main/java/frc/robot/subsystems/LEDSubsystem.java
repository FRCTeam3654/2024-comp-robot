...
public class LEDSubsystem extends SubsystemBase {
  ...
  public void rainbow() {
    ...
    for (var i = 0; i < m_led_left_Buffer.getLength(); i++) {
      ...
      final var hue = (m_rainbowFirstPixelHue_left + (i * 180 / m_led_left_Buffer.getLength())) % 180;
      // Set the value
      m_led_left_Buffer.setHSV(i, hue, 255, 128);
    }
/* 
     for (var i = 0; i < m_led_right_Buffer.getLength(); i++) {
      // Calculate the hue - hue is easier for rainbows because the color
      // shape is a circle so only one value needs to precess
      final var hue = (m_rainbowFirstPixelHue_right + (i * 180 / m_led_right_Buffer.getLength())) % 180;
      // Set the value
      m_led_right_Buffer.setHSV(i, hue, 255, 128);
    }
    */
